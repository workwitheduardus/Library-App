...
export async function getRecommendedBooksApi(
  query: GetRecommendedBooksQuery,
  ...
  const res = await fetch(`${BASE_URL}/api/books/recommend?${params}`);
  if (!res.ok) throw new Error("Failed to fetch recommendations");
  const json = (await res.json()) as ApiResponse<GetRecommendedBooksResponse>;
  return json.data;
}